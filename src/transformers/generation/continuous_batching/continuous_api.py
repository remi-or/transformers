...
class ContinuousBatchProcessor:
    ...
    def reset_static_tensors(self, full_reset: bool = False):
        ...
        c = self.total_past_length if self.slice_inputs and not full_reset else self.read_index.size(-1)
        # Reset the tensors
        ...
        if self.attention_mask is not None:
            self.attention_mask[:, :, :t, :t+c].fill_(torch.finfo(self.model_dtype).min)
    ...
    def get_model_kwargs(self) -> PagedAttentionArgs:
        ...
        c = self.total_past_length if self.slice_inputs else self.read_index.size(-1)
        # Prepare the kwargs
        kwargs = {
        ...
        if self.attention_mask is not None:
            kwargs["attention_mask"] = self.attention_mask[:, :, :t, :t+c]
        return kwargs
    ...
    def prepare_next_batch(self):
        ...
        for state in self.requests_in_batch:
            ...
            self.total_past_length += past_length

            positions_to_add = cache_index[past_length:]
            self.cache.get_read_indices(state.request_id, past_length, read_index)
