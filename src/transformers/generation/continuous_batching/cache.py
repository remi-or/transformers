...
class PagedAttentionCache:
    ...
    def get_read_indices(
        self, request_id: str, past_length: int, read_index: list[list[int]]
    ) -> None:
        ...
        for cm, read_indices in zip(self.group_cache_managers, read_index):
            indices = cm.get_read_indices(request_id, past_length)
    ...
    def update(
        ...
        layer_idx: int,
        read_index: torch.Tensor, # shape [num_layer_groups, past_length]
        ...
        **kwargs,
        ...
        key_states_with_cache = torch.cat([k_cache_flat[layer_idx_in_group, layer_read_index, :, :], key_states], dim=0)
        value_states_with_cache = torch.cat([v_cache_flat[layer_idx_in_group, layer_read_index, :, :], value_states], dim=0)
